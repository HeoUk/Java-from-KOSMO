...
public class ArrayType02 {
	...
	public static void main(String[] args) {
		...
		*/
		...
		System.out.println("[초기화 전]");
		for(int i=0;i < dynamic.length;i++) {//i를 행번호로
			for(int k=0;k < dynamic[i].length;k++) {//k를 열번호로
		...
		*/
		...
		for(int i=0;i < arrInit.length;i++) {//i를 행번호로
			for(int k=0;k < arrInit[i].length;k++) {//k를 열번호로
		...
		/*   
		   ...
		   9 9 9 1 9     
	   */
		//문1]2차원 배열을 선언하는데 ,선언과 
	    //   동시에
	    //   메모리를 할당해라.
		int[][] nine =new int[4][5];
		//문2]위의 값을 문1]번에서 선언한 배열에 저장해라.
		for(int i=0;i < nine.length;i++) {//i를 행번호로
			for(int k=0;k < nine[i].length;k++) {//k를 열번호로
				if(i==k) nine[i][k]=1;
				else nine[i][k]=9;
			}			
		}
