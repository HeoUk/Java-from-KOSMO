...
public class ResultSetMetaDataSQL extends IConnectImpl {
	...
	public ResultSetMetaDataSQL(String url, String user, String password) {
	...
	public void execute() throws Exception {
		try {
			...
			//오라클의 타입명으로 반환
			System.out.println("[오라클의 컬럼 타입명으로 얻기]");
			for(int i=1;i <= columnCount ;i++) {
			...
			for(int i=1;i <= columnCount ;i++) {
				 ...
				 * 
				 */
				String coulumName = rsmd.getColumnName(i).length() >  dashCount.get(i-1) ?
						            rsmd.getColumnName(i).substring(0,dashCount.get(i-1)) :
						            rsmd.getColumnName(i);
				System.out.print(String.format("%-"+(dashCount.get(i-1)+1)+"s", coulumName));
			}//////////////for
	...
	public static void main(String[] args) throws Exception {
		new ResultSetMetaDataSQL(ORACLE_URL, "scott", "scott").execute();;
	}///////////main
